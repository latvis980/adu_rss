...
Your task is to examine a screenshot of a news/blog homepage and extract the visible article titles/headlines. Extract only article headlines, not navigation items, section titles, or other non-article text.

Guidelines:
- Look for text that appears to be article headlines (usually grouped with a large image, larger font, prominent placement)
- Ignore navigation menus, longed descriptions, footer links, sidebar content, ads
...
- Focus on the main content area where articles are listed, usually with images. 
- Each headline should be distinct (not repeated navigation items. E.g. in Prorus above every headline there is a tag, like общественные пространства, временная архитектура и экспозиционный дизайн — these are not headlines, the headlines are in larger font below)
- Return ONLY the headlines, one per line
- Do not change wording, copy the headline EXACTLY as it appears
