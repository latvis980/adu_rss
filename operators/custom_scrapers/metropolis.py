# operators/custom_scrapers/metropolis.py
"""
Metropolis Custom Scraper - Visual AI Approach
Scrapes architecture news from Metropolis Magazine

Site: https://metropolismag.com/projects/
Challenge: User-Agent required
...
class MetropolisScraper(BaseCustomScraper):
    """
    Visual AI-powered custom scraper for Metropolis Magazine
    Uses GPT-4o vision to identify articles on projects page.
    """

    source_id = "metropolis"
    source_name = "Metropolis"
    base_url = "https://metropolismag.com/projects/"
    ...
    async def _find_headline_in_html_with_ai(self, page, headline: str) -> Optional[dict]:
        ...
        html_context = await page.evaluate("""
            (headline) => {
                // Find all article-like containers (including project-specific classes)
                const containers = document.querySelectorAll(
                    'article, .post, [class*="post"], [class*="item"], [class*="card"], [class*="project"], .entry'
                );
        ...
        prompt = f"""You are analyzing article containers from metropolismag.com to find which one matches a target headline.
...
custom_scraper_registry.register(MetropolisScraper)
...
async def test_metropolis_scraper():
    """Test the visual AI scraper."""
    print("=" * 60)
    print("Testing Metropolis Visual AI Scraper")
    print("=" * 60)
    ...
    try:
        ...
        stats = await scraper.tracker.get_stats(source_id="metropolis")
...
if __name__ == "__main__":
    asyncio.run(test_metropolis_scraper())